...
use common::{
    types::{Block, BlockTag, SGXResult},
...
unsafe impl<DB: Database> Sync for RpcInner<DB> {}
...
#[async_trait]
impl<DB: Database> EthSGXRpcServer for RpcInner<DB> {
    async fn get_balance(&self, address: &str, block: BlockTag) -> Result<SGXResult, Error> {
        let address = convert_err(Address::from_str(address))?;
        let balance = convert_err(self.node.get_balance(&address, block).await)?;
...
fn sgx_result(result: String, sign_fn: Option<Arc<Box<dyn Fn(String) -> String>>> ) -> SGXResult{
    SGXResult{
        result: result.clone(),
